    ...
    return function (moduleArg = {}) {
            ...
            getDB: (name, callback) => {
                ...
                req.onupgradeneeded = (e) => {
                    ...
                    if (db.objectStoreNames.contains(IDBFS.DB_STORE_NAME)) {
                        fileStore = transaction.objectStore(
                            IDBFS.DB_STORE_NAME
                        );
                    } else {
        ...
        var Browser = {
            ...
            setMouseCoords(pageX, pageY) {
                const { x: x, y: y } = Browser.calculateMouseCoords(
                    pageX,
                    pageY
                );
        ...
        var GodotRuntime = {
            ...
            allocStringArray: function (p_strings) {
                ...
                for (let i = 0; i < size; i++) {
                    HEAP32[(c_ptr >> 2) + i] = GodotRuntime.allocString(
                        p_strings[i]
                    );
                }
                return c_ptr;
                ...
                return new Promise(function (resolve, reject) {
                    FS.syncfs(true, function (err) {
                        if (err) {
                            ...
                            GodotFS._idbfs = false;
                            GodotRuntime.print(
                                `IndexedDB not available: ${err.message}`
                            );
                        } else {
                            GodotFS._idbfs = true;
                        }
        ...
        var GodotAudio = {
            ...
            Sample: class Sample {
                ...
                _duplicateAudioBuffer() {
                    if (this._audioBuffer == null) {
                        throw new Error(
                            "couldn't duplicate a null audioBuffer"
                        );
                    }
                    ...
                    for (
                        let i = 0;
                        i < this._audioBuffer.numberOfChannels;
                        i++
                    ) {
            ...
            SampleNodeBus: class SampleNodeBus {
                ...
                constructor(bus) {
                    ...
                    this._channelSplitter =
                        GodotAudio.ctx.createChannelSplitter(
                ...
                setVolume(volume) {
                    ...
                    this._l.gain.value =
                        volume[GodotAudio.GodotChannel.CHANNEL_L] ?? 0;
                    this._r.gain.value =
                        volume[GodotAudio.GodotChannel.CHANNEL_R] ?? 0;
                    this._sl.gain.value =
                        volume[GodotAudio.GodotChannel.CHANNEL_SL] ?? 0;
                    this._sr.gain.value =
                        volume[GodotAudio.GodotChannel.CHANNEL_SR] ?? 0;
                    this._c.gain.value =
                        volume[GodotAudio.GodotChannel.CHANNEL_C] ?? 0;
            ...
            SampleNode: class SampleNode {
                ...
                static stopSampleNode(id) {
                    const sampleNode =
                        GodotAudio.SampleNode.getSampleNodeOrNull(id);
                    if (sampleNode == null) {
                        return;
                    }
                    sampleNode.stop();
                ...
                constructor(params, options = {}) {
                    ...
                    this.connectPositionWorklet(options.start).catch((err) => {
                        const newErr = new Error(
                            "Failed to create PositionWorklet."
                        );
                        newErr.cause = err;
                ...
                setVolumes(buses, volumes) {
                    for (let busIdx = 0; busIdx < buses.length; busIdx++) {
                        const sampleNodeBus = this.getSampleNodeBus(
                            buses[busIdx]
                        );
                ...
                getSampleNodeBus(bus) {
                    if (!this._sampleNodeBuses.has(bus)) {
                        const sampleNodeBus =
                            GodotAudio.SampleNodeBus.create(bus);
                        ...
                        switch (event.data["type"]) {
                            case "position":
                                this._playbackPosition =
                                    parseInt(event.data.data, 10) /
                                        this.getSample().sampleRate +
                                    this.offset;
                                break;
                            default:
                        }
                            ...
                            case "disabled":
                                {
                                    ...
                                    self.stop();
                                    if (
                                        GodotAudio.sampleFinishedCallback !=
                                        null
                                    ) {
                                        const idCharPtr =
                                            GodotRuntime.allocString(id);
                                        GodotAudio.sampleFinishedCallback(
                                            idCharPtr
                                        );
            ...
            Bus: class Bus {
                ...
                static move(fromIndex, toIndex) {
                    ...
                    const buses = GodotAudio.buses.filter(
                        (_, i) => i !== fromIndex
                    );
                ...
                clear() {
                    GodotAudio.buses = GodotAudio.buses.filter(
                        (v) => v !== this
                    );
                }
                _syncSampleNodes() {
            ...
            init: function (mix_rate, latency, onstatechange, onlatencyupdate) {
                ...
                const ctx = new (window.AudioContext ||
                    window.webkitAudioContext)(opts);
                GodotAudio.ctx = ctx;
            ...
            create_input: function (callback) {
                ...
                function gotMediaInput(stream) {
                    try {
                        GodotAudio.input =
                            GodotAudio.ctx.createMediaStreamSource(stream);
        ...
        function _godot_audio_get_sample_playback_position(
        ...
        function _godot_audio_has_script_processor() {
            return GodotAudio.ctx && GodotAudio.ctx.createScriptProcessor
                ? 1
                : 0;
        }
        ...
        function _godot_audio_sample_register_stream(
            ...
            const subLeft = GodotRuntime.heapSub(
                HEAPF32,
                framesPtr,
                framesTotal
            );
        ...
        function _godot_audio_sample_set_finished_callback(callbackPtr) {
            GodotAudio.sampleFinishedCallback =
                GodotRuntime.get_func(callbackPtr);
        }
        ...
        function _godot_audio_sample_stream_is_registered(
            streamObjectIdStrPtr
        ) {
            const streamObjectId =
                GodotRuntime.parseString(streamObjectIdStrPtr);
        ...
        var GodotAudioScript = {
            ...
            start: function (
                ...
                GodotAudioScript.script.onaudioprocess = function (event) {
                    const inb = GodotRuntime.heapSub(
                        HEAPF32,
                        p_in_buf,
                        p_in_size
                    );
                    ...
                    onprocess();
                    const outb = GodotRuntime.heapSub(
                        HEAPF32,
                        p_out_buf,
                        p_out_size
                    );
        ...
        function _godot_js_display_screen_size_get(width, height) {
            ...
            GodotRuntime.setHeapValue(
                width,
                window.screen.width * scale,
                "i32"
            );
            GodotRuntime.setHeapValue(
                height,
                window.screen.height * scale,
                "i32"
            );
        }
        ...
        function _godot_js_fetch_create(
            ...
            const headers = GodotRuntime.parseStringArray(
                p_headers,
                p_headers_size
            );
        ...
        var GodotIME = {
            ...
            ime_active: function (active) {
                ...
                if (active) {
                    ...
                    GodotIME.focusTimerIntervalId = setInterval(
                        focusTimer,
                        100
                    );
                } else {
                ...
                function ime_event_cb(event) {
                    ...
                    switch (event.type) {
                        ...
                        case "compositionupdate":
                            {
                                const ptr = GodotRuntime.allocString(
                                    event.data
                                );
                                ime_cb(1, ptr);
                                GodotRuntime.free(ptr);
        ...
        function _godot_js_rtc_datachannel_connect(
            ...
            const onmessage = GodotRuntime.get_func(p_on_message).bind(
                null,
                p_ref
            );
        ...
        function _godot_js_rtc_datachannel_send(
            p_id,
            p_buffer,
            p_length,
            p_raw
            ...
            create: function (
                ...
                if (
                    ...
                    conn.onconnectionstatechange = function (event) {
                        ...
                        onConnectionChange(
                            GodotRTCPeerConnection.ConnectionState[
                ...
                conn.onicegatheringstatechange = function (event) {
                    ...
                    onIceGatheringChange(
                        GodotRTCPeerConnection.IceGatheringState[
                ...
                conn.onsignalingstatechange = function (event) {
                    ...
                    onSignalingChange(
                        GodotRTCPeerConnection.SignalingState[
                            conn.signalingState
                        ] || 0
                    );
                };
        ...
        function _godot_js_rtc_pc_local_description_set(
            ...
            ref.setLocalDescription({ sdp: sdp, type: type }).catch(function (
                error
            ) {
        ...
        function _godot_js_rtc_pc_offer_create(
            ...
            const onsession = GodotRuntime.get_func(p_on_session).bind(
                null,
                p_obj
            );
            const onerror = GodotRuntime.get_func(p_on_error).bind(null, p_obj);
                    ...
                    input.addEventListener(
                        ...
                        (event) => {
                            ...
                            if (size > dataBufferLen) {
                                throw new Error(
                                    `data too big ${size} > ${dataBufferLen}`
                                );
                            }
        ...
        var GodotWebSocket = {
            ...
            create: function (
                ...
                socket.onopen = GodotWebSocket._onopen.bind(
                    null,
                    id,
                    p_on_open
                );
                ...
                socket.onerror = GodotWebSocket._onerror.bind(
                    null,
                    id,
                    p_on_error
                );
                socket.onclose = GodotWebSocket._onclose.bind(
                    null,
                    id,
                    p_on_close
                );
        ...
        function _godot_js_websocket_create(
            ...
            const on_message = GodotRuntime.get_func(p_on_message).bind(
                null,
                p_ref
            );
            const on_error = GodotRuntime.get_func(p_on_error).bind(
                null,
                p_ref
            );
            const on_close = GodotRuntime.get_func(p_on_close).bind(
                null,
                p_ref
            );
            const url = GodotRuntime.parseString(p_url);
        ...
        var GodotJSWrapper = {
            ...
            variant2js: function (type, val) {
                switch (type) {
                    ...
                    case 2: {
                        const heap_value = GodotRuntime.getHeapValue(
                            val,
                            "i64"
                        );
        ...
        function _godot_js_wrapper_object_get(p_id, p_exchange, p_prop) {
            ...
            if (p_prop) {
                ...
                try {
                    ...
                    GodotRuntime.error(
                        `Error getting variable ${prop} on object`,
                        obj
                    );
                    return 0;
                }
        ...
        function _godot_js_wrapper_object_set(
            p_id,
            p_name,
            p_type,
            p_exchange
        ...
        function _godot_js_wrapper_object_set_cb_ret(p_val_type, p_val_ex) {
            GodotJSWrapper.cb_ret = GodotJSWrapper.variant2js(
                p_val_type,
                p_val_ex
            );
        }
        ...
        var GodotWebXR = {
            ...
            getLayer: () => {
                ...
                layer = GodotWebXR.gl_binding.createProjectionLayer({
                    textureType:
                        new_view_count > 1 ? "texture-array" : "texture",
        ...
        function _godot_webxr_get_frame_rate() {
            if (
                !GodotWebXR.session ||
                GodotWebXR.session.frameRate === undefined
            ) {
                return 0;
            }
        ...
        function _godot_webxr_get_supported_frame_rates(r_frame_rates) {
            ...
            const frame_rate_count =
                GodotWebXR.session.supportedFrameRates.length;
        ...
        var _godot_webxr_initialize = function (
            ...
            const required_features = GodotRuntime.parseString(
                p_required_features
            )
                .split(",")
                .map((s) => s.trim())
                .filter((s) => s !== "");
            const optional_features = GodotRuntime.parseString(
                p_optional_features
            )
                .split(",")
                .map((s) => s.trim())
            ...
            navigator.xr
                ...
                .then(function (session) {
                    ...
                    gl.makeXRCompatible()
                        .then(function () {
                            GodotWebXR.gl_binding = new XRWebGLBinding(
                                session,
                                gl
                            );
                            GodotWebXR.getLayer();
                            ...
                            function requestReferenceSpace() {
                                ...
                                session
                                    ...
                                    .catch(() => {
                                        if (
                                            requested_reference_space_types.length ===
                                            0
                                        ) {
                                            const c_str =
                                                GodotRuntime.allocString(
        ...
        function _godot_webxr_update_input_source(
            ...
            const target_pose = frame.getPose(
                input_source.targetRaySpace,
                space
            );
            if (!target_pose) {
                return false;
            ...
            if (input_source.gamepad) {
                ...
                button_count = Math.min(
                    input_source.gamepad.buttons.length,
                    10
                );
        ...
        for (var i = 0; i <= 288; ++i) {
            miniTempWebGLIntBuffers[i] =
                miniTempWebGLIntBuffersStorage.subarray(0, i);
        }
        ...
        var ___wasm_call_ctors = () =>
            (___wasm_call_ctors = wasmExports["jf"])();
        var _free = (a0) => (_free = wasmExports["kf"])(a0);
...
const Preloader = /** @constructor */ function () {
    ...
    function loadFetch(file, tracker, fileSize, raw) {
        ...
        return fetch(file).then(function (response) {
            if (!response.ok) {
                return Promise.reject(
                    new Error(`Failed loading file '${file}'`)
                );
            }
    ...
    this.preload = function (pathOrBuffer, destPath, fileSize) {
        ...
        if (typeof pathOrBuffer === "string") {
            const me = this;
            return this.loadPromise(pathOrBuffer, fileSize).then(function (
                buf
            ) {
...
const InternalConfig = function (initConfig) {
    ...
    Config.prototype.getModuleConfig = function (loadPath, response) {
        ...
        return {
            ...
            noExitRuntime: false,
            dynamicLibraries: [`${loadPath}.side.wasm`].concat(
                this.gdextensionLibs
            ),
